...
int main(int argc, char* argv[]) 
{
 ...
 for (size_t i2 = 0; i2 < 3; i2++)
    ...
    for(int n=1;n<=max_outer_iterations;n++) {
       ...
       MPI_Barrier(MPI_COMM_WORLD);
        //1离散动量方程 
        movement_function(mesh,equ_u,equ_v,1000);
       ...
       /*if((l2x/mesh.internumber) < 1e-8 & (l2y/mesh.internumber) < 1e-8 & (l2p/mesh.internumber) < 1e-9) { 
    ...
    Mesh final_mesh = mergeMeshesWithoutInterface(sub_meshes);
    post_processing(final_mesh,final_mesh.nx,final_mesh.ny,1);
    return 0;
}
